...
**Surefire poms that hardcode `<argLine>` will silently override `-DargLine=` from the command line.** If `-Dmaven.surefire.debug` doesn't open port 5005, change the pom's `<argLine>X</argLine>` to `<argLine>X ${argLine}</argLine>` and add `<argLine></argLine>` to `<properties>` as the empty default.
...
2. **Attach:** `jdwp_wait_for_attach()` — polls until the JVM is listening, then attaches.
...
5. **Inspect everything in one call:** `jdwp_get_breakpoint_context()` — returns thread, top frames, locals (incl. `this`), and `this` field dump.
...
### Setup / lifecycle
- **`jdwp_wait_for_attach(host?, port?, timeoutMs?)`** — poll until the JVM is listening, then attach. Use this as the FIRST call after launching the target. Defaults: `localhost`, `5005`, `30000ms`.
- **`jdwp_connect`** — attach immediately (fails if no JVM is listening). Use when the JVM is already known to be up; otherwise prefer `jdwp_wait_for_attach`.
...
### Inspection
- **`jdwp_get_breakpoint_context(maxFrames?, includeThisFields?)`** — one-shot context dump at the current BP: thread, top frames (junit/maven/reflection collapsed), frame-0 locals (with synthetic `this`), and the `this` field dump. The single-call shortcut for "what is the state right now?". Defaults: 5 frames, fields included.
...
- **`jdwp_get_locals(threadId, frameIndex)`** — visible local variables (frame 0 = topmost). For instance methods, `this` is exposed as a synthetic local — no need to `eval this` separately.
...
- **`jdwp_evaluate_expression(threadId, expression, frameIndex?)`** — eval any Java expression in scope. Primitive return values are unboxed and printed directly (e.g. `eval order.getTotal()` → `71.982`).
...
### Breakpoints
- **`jdwp_set_breakpoint(className, lineNumber, suspendPolicy?, condition?)`** — line BP. `suspendPolicy` defaults to `"all"`; `condition` is optional.
- **`jdwp_set_logpoint(className, lineNumber, expression, condition?)`** — non-stopping BP that records the expression result to event history. Optional `condition` (Java boolean expression) gates the log: only fires when the condition is true. Both expression and condition see the same locals/fields as `evaluate_expression`.
...
### Execution control
- **`jdwp_resume_until_event(timeoutMs?)`** — resume the VM and BLOCK until the next BP/step/exception event fires (or timeout). The standard way to advance execution between events. Default timeout: 30000ms.
...
Each recipe matches a class of bug.
...
**Approach C — conditional logpoint (combines A and B):**
- `set_logpoint("MyClass", 42, "\"i=\" + i + \" v=\" + value", condition="value < 0 || items.size() > 50")`
- Logs only when the suspicious shape appears, so `get_events` returns just the relevant frames instead of every iteration. Cheaper than logging the whole loop, more informative than a plain conditional BP.
...
**Approach:**
1. Launch the target in a background shell with `suspend=y` (e.g. `mvn test -Dtest=MyTest -Dmaven.surefire.debug`, `./gradlew test --debug-jvm`, or a manual `-agentlib:jdwp=...` on a JVM you launch directly).
2. **`jdwp_wait_for_attach()`** — polls every 200ms until the JVM is listening, then attaches. No manual port-checking required.
3. `jdwp_set_breakpoint(className, lineNumber)` — set your trap.
...
For a follow-up scenario against the same target, finish step 5 with `jdwp_resume_until_event` (advance to the next BP) or `jdwp_reset` + new breakpoints (start a fresh investigation without re-attaching).

## Gotchas

- **`evaluate_expression` auto-rewrites bare references to fields of `this`** (e.g. `sessions.containsKey(k)` → `_this.sessions.containsKey(k)`) when the enclosing class is public. For PACKAGE-PRIVATE enclosing classes the rewrite is skipped (the wrapper class can't see the type) — the error message will name the field and tell you to use `jdwp_get_fields(<thisObjectId>)` instead. The `this` object ID is in the regular `jdwp_get_locals` output (synthetic `this` entry), so this is a one-call workaround.
...
3. **Free port 5005:** orphaned test JVMs can hold the JDWP port. `ps -ef | grep jdwp=transport` then `kill <pid>`.
4. **Both processes gone after a crash:** the test JVM exits when its debugger detaches; the MCP server may also have crashed. Reconnect MCP and relaunch the test JVM from scratch.
