...
mvn clean package -DskipTests      # Full build (produces target/mcp-jdwp-java-1.0.0.jar)
```

There are no tests in this project. The `-DskipTests` flag is standard practice here.

The compiler requires `--add-modules jdk.jdi` (configured in pom.xml). At runtime, the JAR must also be launched with `--add-modules jdk.jdi`.
